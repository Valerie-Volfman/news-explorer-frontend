...
function SavedNewsHeader() {
  ...
  return (
    <div className="header-news">
      ...
      <h2 className="header-news__info">{`${currentUser.name}, you have ${currentUser.articles && currentUser.articles.length} saved articles`}</h2>
      <p className="header-news__keywords">
        By keywords:{" "}
        <b className="header-news__keywords_bold">
        {keywords.length ?` ${keywords[0]}${keywords.length > 1 ? ", " + keywords[1] : ""}${
